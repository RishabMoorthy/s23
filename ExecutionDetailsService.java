...
public class ExecutionDetailsService {
    ...
    private void createRecordsFromVsFile(ServiceConfig config, VsCatalog catalog, String virtServer) {
        ...
        // A .vs with an empty <ExecutionModeValue/> is recorded as "Disabled": the feature is
        // off for this service. A blank value in the portal is ambiguous - users cannot tell
        // whether it is off, unconfigured or broken - so the state is made explicit here. The
        // .vs file itself is still left blank; only the database carries the name.
        String mode = trimToEmpty(model.getExeModeValue());
        if (mode.isEmpty()) {
            mode = ExecutionModeType.DISABLED.value();
        }
